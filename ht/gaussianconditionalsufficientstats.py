...
class GaussianEstimator(UnivariateNormalEstimator):
    ...
    def weight_less_than_equal_and_greater_than(self, value):
        '''
        给定分裂属性的分裂值，利用概率密度返回结点数据中的大于该值，等于该值，小于该值的权重和。
        :param value:
        :return:
        '''
        std_dev = math.sqrt(self._variance)
        ...
        if std_dev > 0:
            less_w = utils.normal_probability(
                (value - self._mean) / std_dev) * self._sum_of_weights - equal_w    #这里返回的是小于等于的概率
        ...
        if min_value < math.inf:
            ...
            for i in range(self._num_bins):
                ...
                if split > min_value and split < max_value:
                    splits.add(split)
#SortedList([23.636363636363637, 30.272727272727273, 36.90909090909091, 43.54545454545455, 50.18181818181819, 56.81818181818182, 63.45454545454545, 70.0909090909091, 76.72727272727273, 83.36363636363637])
        return splits
    ...
    def _class_dists_after_split(self, split_val):
        ...
        for class_val, att_estimator in self._class_lookup.items():
            if att_estimator is not None:
                if split_val < self._min_val_observed_per_class[class_val]:#分裂值小于最小值
                    mass = rhs_dist.get(class_val, None)
                    if mass is None:
                        mass = WeightMass()
                        rhs_dist[class_val] = mass
                    mass.weight += att_estimator.get_sum_of_weights()
                elif split_val > self._max_val_observed_per_class[class_val]:#分裂值大于最大值
