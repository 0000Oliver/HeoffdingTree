...
class Dataset(object):
	...
	def __init__(self, attributes, class_index=-1, instances=None, name='New dataset'):
		...
		self.__distubute = [0]*self.__attributes[class_index].num_values()
	...
	def add(self, instance):
		...
		self.__distubute[instance.class_value()]+=1
	...
	def get_distribute(self):
		distubute={}
		for i in range(len( self.__distubute)):
			distubute[self.__attributes[self.__class_index].all_values()[i]] = self.__distubute[i]
		return distubute
